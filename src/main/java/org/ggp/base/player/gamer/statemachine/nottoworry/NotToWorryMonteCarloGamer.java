...
public class NotToWorryMonteCarloGamer extends NotToWorryGamer {
	private long start;
	private int maxTime;
	...
	public int minScore(Role role, MachineState state, int alpha, int beta, int turn, int depth)
		...
		for (int i = 0; i < actions.size(); i++) {
			...
			for (int j = 0; j < jointActions.size(); j++) {
				...
				if (turn == getStateMachine().getRoles().size() - 1) {
					result = maxScore(role, newState, alpha, beta, depth);
				} else {
					result = minScore(role, newState, alpha, beta, turn + 1, depth);
				}
	...
	public int maxScore(Role role, MachineState state, int alpha, int beta, int depth)
		...
		for (int i = 0; i < actions.size(); i++) {
			...
			for (int j = 0; j < jointActions.size(); j++) {
				List<Move> moves = jointActions.get(j);
				MachineState newState = getStateMachine().getNextState(state, moves);
				int result = minScore(role, newState, alpha, beta, 1, depth+1);
	...
	public int MonteCarlo(Role role, MachineState state, int count) {
		int total = 0;
	...
	public int DepthCharge(Role role, MachineState state) {
		if (getStateMachine().isTerminal(state)) {
			int goal = 0;
			try {
				goal = getStateMachine().getGoal(state, role);
		...
		for (int i = 0; i < roles.size(); i++) {
			...
			try {
				...
				int randNum = (int)(Math.random() * (actions.size()-1));
				moves.set(i, actions.get(randNum));
		...
		try {
			newState = getStateMachine().getNextState(state,moves);
	...
	private Move selectMoveAlphabeta(List<Move> actions) throws MoveDefinitionException, TransitionDefinitionException, GoalDefinitionException {
		...
		for (int i = 0; i < actions.size(); i++) {
			...
			for (int j = 0; j < jointActions.size(); j++) {
				...
				int result = minScore(getRole(), newState, 0, 100, turn, 0);
				if (result == 100) {
